...
class CVALSTrainer:
    ...
    def train(self, key: jax.random.PRNGKey):
        model = self.model
        opt_diffusion_state = self.optimizer_diffusion.init(eqx.filter(model, eqx.is_array))
        ...
        for batch_index, batch in enumerate(pbar):
            ...
            if (batch_index // self.switch_steps) % 2 == 0:
                ...
                if batch_index % self.log_every_n_steps == 0:
                    ...
                    self.logger.add_scalar("learning_rate_stein", opt_stein_state.hyperparams["learning_rate"].item())
                ...
                if batch_index % self.log_every_n_steps == 0:
                    ...
                    self.logger.add_scalar("learning_rate_diffusion", opt_diffusion_state.hyperparams["learning_rate"].item())
